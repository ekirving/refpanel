...
ruleorder: hgdp_filter_iupac_base_codes > sgdp_download_cram > ggvp_filter_iupac_base_codes > samtools_cram
...
rule refpanel:
    input:
        ...
        expand("data/panel/{panel}/vcf/{panel}_chrALL_vqsr_norm_annot_filter_mendel.vcf.gz", panel=config["refpanel"]),
